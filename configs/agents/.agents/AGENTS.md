...
- A copied skill should still work outside its source workspace with only its own files.
- Put runtime dependencies and standalone scripts in the skill package, not the workspace root.
- Keep the root `package.json`, `eslint.config.ts`, and `tsconfig.json` limited to shared authoring tooling.
